...
def view_hull_mod(starsector_data_dir, hull_mod, key_by):
    json_tree(shipper.get_hull_mods(starsector_data_dir, key_by).get(hull_mod, {}), "Hull Mod")

def view_ship_system(starsector_data_dir, ship_system, key_by):
    json_tree(shipper.get_ship_systems(starsector_data_dir, key_by).get(ship_system, {}), "Ship System")